    ...
    @component('admin.components.breadcumb',['secondaryactive' => 'active'])
        ...
        <div class="col-md-7 col-lg-6">
            <div class="widget-button">
                ...
                @endcan
                @can('users.create')
                <a href="{{ route('users.import') }}" type="button" title=" {{ __('Import') }}"
                    class="btn btn-info p-2"> <i class="flaticon-import"></i>
                    {{ __('Import') }}</a>
                @endcan
                <a href="{{ route('users.trash') }}" type="button" class="btn btn-success"
                        ...
                        <table class="table data-table display" id="example">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (!empty($data->image))
                                        <img src="{{ asset('/images/users/' . $data->image) }}" alt=" {{ __('user img') }}"
                                            class="widget-img">
                                        @else
                                        <img src="{{ Avatar::create($data->name)->toBase64() }}" alt=" {{ __('user img') }}">
                                        @endif
                                    </td>
                                    <td>
                                        ...
                                        <ul>
                                            ...
                                            <li><strong>{{ __('Email:') }}</strong><a href="mailto:{{ $data->email }}"
                                                    title="{{ $data->email }}" class="email-tag">
                                                    {{ $data->email }}</a></li>
                                            <li><strong>{{ __('Mobile:') }}</strong><a href="tel: {{ $data->mobile }}" class="mobile-tag" title="{{ $data->mobile }}">
                                                    {{ $data->mobile }}</a>
                                            </li>
                                        </ul>
                                    </td>

                                    <td>{{ $data->role_name ?? 'No role' }}</td>
                                    <td>
                                        @if ($data->role != 'A')
                                        <a href="{{ route('userslogin', ['id' => $data->id]) }}" title="{{__('Login')}}">
                                            <i class="flaticon-key rating-icon-color fa-2x"></i>
                                        </a>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="dropdown action-dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    @can('users.edit')
                                                    <a href="{{ url('admin/users/' . $data->id . '/edit') }}"
